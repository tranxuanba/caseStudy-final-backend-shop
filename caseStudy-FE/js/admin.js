...
function displayProduct(product) {
    return `<tr><td>${product.name}</td><td>${product.price}</td><td>${product.quantity}</td><td>${product.weight}</td>
            <td>${product.description}</td><td><img src="/caseStudy-FE/img/${product.image}" alt="Lỗi"></td><td>${product.category.name}</td>
...
function searchProduct() {
    ...
    $.ajax({
        type: "GET",
        url: `http://localhost:8095/api/products/search?search=${search}`,
        success: function (data) {
            let content = displayProductHeader();
            for (let i = 0; i < data.length; i++) {
                content += displayProduct(data[i]);
...
function getOrdersByPage() {
    $.ajax({
        type: "GET",
        url: `http://localhost:8095/api/orders`,
        success: function (data) {
